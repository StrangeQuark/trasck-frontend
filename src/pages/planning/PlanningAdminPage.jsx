...
import { StatusOptionChecklist } from '../../components/StatusOptionChecklist';
import { TextField } from '../../components/TextField';
import { useApiAction } from '../../hooks/useApiAction';
import { csv, firstId, numberOrUndefined } from '../../utils/forms';

export const PlanningAdminPage = ({ context }) => {
  ...
  const [boardStatusOptions, setBoardStatusOptions] = useState([]);
  ...
  const [boardForm, setBoardForm] = useState({ name: 'Sprint Board', type: 'scrum', teamId: '', projectScoped: 'true' });
  const [columnForm, setColumnForm] = useState({ name: 'Ready', position: '1', wipLimit: '', doneColumn: 'false', statusIds: [] });
  ...
  const load = async () => {
    ...
    const result = await action.run(async () => {
      ...
      const [columnRows, swimlaneRows, boardCards, releaseItems, roadmapRowsForSelected, statusOptions] = await Promise.all([
        ...
        canReadProject && nextBoardId ? context.services.planning.listBoardStatusOptions(nextBoardId) : Promise.resolve([]),
      ]);
      return {
        teamRows,
        projectTeamRows,
    ...
    if (result) {
      ...
      setBoardStatusOptions(result.statusOptions || []);
  ...
  const createColumn = async (event) => {
    ...
    setColumnForm({ ...columnForm, name: '', statusIds: [] });
  ...
  const loadBoardDetails = async () => {
    ...
    if (result) {
      const [columnRows, swimlaneRows, cardRows, statusOptions] = result;
      setBoardColumns(columnRows || []);
      setBoardSwimlanes(swimlaneRows || []);
      setBoardWorkItems(cardRows || null);
      setBoardStatusOptions(statusOptions || []);
    }
  };
    ...
    <div className="planning-layout">
      ...
      <Panel title="Board Layout" icon={<FiSliders />}>
        <form className="stack" onSubmit={createColumn}>
          ...
          <StatusOptionChecklist options={boardStatusOptions} value={columnForm.statusIds} onChange={(statusIds) => setColumnForm({ ...columnForm, statusIds })} />
